...
def cfg_path(key):
    ...
    if key == "workspace":
        return WS_ROOT
    if key == "results":
        # Prefer relative resolution so docs/machines stay portable
        rel = paths.get("results_relative", "results")
        p = Path(rel)
        if not p.is_absolute():
            return (WS_ROOT / p).resolve()
        # Legacy absolute path support
        return Path(paths.get("results", WS_ROOT / "results"))
...
def ensure_dirs():
    for sub in [
        "datasets/processed", "datasets/splits",
