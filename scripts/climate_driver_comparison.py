...
def load_frequency_data():
    enso = load_csv("enso_frequency/summary.csv").set_index("Region")
    iod = load_csv("iod_frequency/summary.csv").set_index("Region")
    mei = load_csv("mei_frequency/summary.csv").set_index("Region")
...
def load_lag_data():
    ...
    for driver, prefix in [("ENSO", "enso"), ("IOD", "iod"), ("MEI", "mei")]:
        for region in REGIONS:
            f = f"{prefix}_lag/{region.lower()}_lag_correlation.csv"
            df = load_csv(f)
...
def load_statistics_data():
    ...
    lag_files = {
        "ENSO": "enso_lag/{r}_enso_lag.csv",
        "IOD": "iod_lag/{r}_iod_lag.csv",
        "MEI": "mei_lag/{r}_mei_lag.csv",
    }

    for driver, prefix in [("ENSO", "enso"), ("IOD", "iod"), ("MEI", "mei")]:
        summary_path = RESULTS / f"{prefix}_statistics/summary.csv"
        has_summary = summary_path.exists()

        for region in REGIONS:
            desc[(driver, region)] = load_csv(
                f"{prefix}_statistics/csv/{region.lower()}_descriptive.csv"
            )

            if has_summary:
                summary = load_csv(f"{prefix}_statistics/summary.csv")
...
def load_annual_data():
    ...
    for driver, prefix, idx_col in [
        ...
        for region in REGIONS:
            df = load_csv(f"{prefix}_annual/csv/{region.lower()}_annual.csv")
...
def load_seasonal_data():
    ...
    season_tables = {}
    for driver, prefix in [("ENSO", "enso"), ("IOD", "iod"), ("MEI", "mei")]:
        summary = load_csv(f"{prefix}_seasonal/summary.csv")
        for region in REGIONS:
            s = summary[summary["Region"] == region].iloc[0]
            rows.append({
                "Region": region,
            ...
            season_tables[(driver, region)] = load_csv(
                f"{prefix}_seasonal/csv/{region.lower()}_season_phase.csv",
                index_col=0,
            )
    ...
    for driver, prefix in [("ENSO", "enso"), ("IOD", "iod"), ("MEI", "mei")]:
        for region in REGIONS:
            strength[(driver, region)] = load_csv(
                f"{prefix}_strength/csv/{region.lower()}_strength.csv"
            )
    return strength
...
def plot_frequency_figures(freq):
    ...
    # Percentage comparison for each phase type
    ...
    # Stacked frequency all drivers per region
    for region in REGIONS:
...
def plot_lag_figures(lag_df, lag_curves):
    ...
    # Combined 3-region duration lag
    ...
    # Combined 3-region intensity lag
    fig, axes = plt.subplots(3, 1, figsize=(10, 12), sharex=True)
    for ax, region in zip(axes, REGIONS):
        for driver in DRIVERS:
            c = lag_curves[(driver, region)]
...
def plot_heatmaps(freq, lag, stats, annual, seasonal, rankings):
    ...
    # Master significance matrix (binary)
    sig_matrix = []
    for region in REGIONS:
        row = []
        for driver in DRIVERS:
...
def plot_annual_figures(annual_ts):
    ...
    # Overlay all drivers on one plot per region (normalized index)
    for region in REGIONS:
...
def plot_strength_figures(strength):
    ...
    # Collapsed warm/neutral/cool strength
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    for ax, region in zip(axes, REGIONS):
        warm, neutral, cool = [], [], []
        for driver in DRIVERS:
...
def plot_dashboards(freq, lag, rankings, lag_curves):
    ...
    # Row 0: frequency p-value heatmap inline
