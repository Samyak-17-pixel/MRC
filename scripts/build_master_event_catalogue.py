...
Output:
    results/master_event_catalogue/
        csv/
        figures/
"""
...
def load_sst_stats(region):
    sst = pd.read_csv(RESULTS / f"{region}_bob_sst.csv")
    sst["Date"] = pd.to_datetime(sst["Date"])
    hobday = pd.read_csv(RESULTS / f"climatology/{region}_hobday.csv")
    return sst, hobday
...
def build_region_catalogue(region):
    ...
    base = pd.read_csv(RESULTS / f"mhw_catalogue/{region}_mhw_catalogue.csv")
    ...
    merges = [
        ...
        (f"wind_analysis/{region}_wind_climatology_analysis.csv", "Start_Date", {
            "Wind_Climatology": "Wind_Climatology_ms",
            "Wind_Anomaly": "Wind_Anomaly_ms",
            "Classification": "Wind_Classification",
        }),
        (f"heat_flux_analysis/{region}_heat_flux_analysis.csv", "Start_Date", {
...
def plot_parameter_heatmap(df, region):
    ...
    # Phase & flag heatmap — build matrix directly to avoid index misalignment
    col_labels = []
    matrix_cols = []
...
def plot_timeline(df, region):
    ...
    # Timeline colored by wind classification
    fig, ax = plt.subplots(figsize=(16, max(5, 0.3 * len(df))))
...
def plot_dashboard(df, region):
    ...
    # 1: Duration vs Intensity scatter
...
def plot_combined_overview(all_df):
    ...
    # Combined parameter summary stats table
    summary_rows = []
...
def main():
    ...
    for region in REGIONS:
        ...
        # Category CSVs for easier reading
