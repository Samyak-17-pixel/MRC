#!/usr/bin/env python3
"""
IOD Seasonal Analysis
---------------------
MHW distribution by meteorological season and IOD phase.

Input:
    results/iod_lag/*_iod_lag.csv

Output:
    results/iod_seasonal/
"""
...
OUT = "/home/samyak/mrc_ws/results/iod_seasonal"
...
for region, file in CATALOGUES.items():
    ...
    table = pd.crosstab(df["Season"], df["IOD_Phase"])
    ...
    # Drop IOD phases with zero events (avoids chi-square failure)
    ...
    ax = table.plot(kind="bar", stacked=True, figsize=(10, 6),
                    color=["firebrick", "gray", "royalblue"])
    ax.set_ylabel("MHW Events")
    ax.set_xlabel("Season")
    ax.set_title(f"{region}: Seasonal Distribution by IOD Phase", fontweight="bold")
    ...
    ax.set_ylabel("MHW Events")
    ax.set_xlabel("Season")
    ax.set_title(f"{region}: Seasonal IOD Phase Comparison", fontweight="bold")
    ax.legend()
