...
Output:
  results/publication/figures/07_top5_triptychs/
    longest/
    strongest/
"""
...
def during_dates_peak_centered(start: pd.Timestamp, end: pd.Timestamp, region: str) -> list[pd.Timestamp]:
    ...
    """
    ts = pd.read_csv(RESULTS / f"{region.lower()}_bob_sst.csv")
    ts["Date"] = pd.to_datetime(ts["Date"])
    hob = pd.read_csv(RESULTS / f"climatology/{region.lower()}_hobday.csv")[["DOY", "Threshold90"]]
...
def main():
    ...
    def process_set(df: pd.DataFrame, category: str):
        ...
        for rank, (_, row) in enumerate(df.iterrows(), 1):
            ...
            # consistent per-event SST scaling
            vmin = float(min(b.min(), d.min(), a.min()))
            vmax = float(max(b.max(), d.max(), a.max()))

            # anomaly against DOY climatology (average the 5 climatology days)
            def mean_clim(dates):
