...
def fig_annual_event_counts():
    # Annual stats exist per region.
    files = {
        "North": RESULTS / "annual_statistics/north_annual_stats.csv",
        "Central": RESULTS / "annual_statistics/central_annual_stats.csv",
        "South": RESULTS / "annual_statistics/south_annual_stats.csv",
    }
...
def fig_ml_best_f1_heatmap():
    best = load_csv(BASE / "mhw_ml/outputs/metrics/best_models.csv")
    # best_models.csv includes mixed rows; filter relevant columns
...
def dashboard_one_page(master_summary: pd.DataFrame, driver_rankings: pd.DataFrame):
    ...
    # Panel C: driver composite heatmap
    pivot = driver_rankings.pivot(index="Region", columns="Driver", values="CompositeScore")
    pivot = pivot.loc[["North", "Central", "South"], ["ENSO", "IOD", "MEI"]]
    ...
    # Panel D: annual event counts (all regions)
    ax4 = fig.add_subplot(gs[1, 0:2])
    for region, color in [("North", "#2ecc71"), ("Central", "#3498db"), ("South", "#e74c3c")]:
        annual = load_csv(RESULTS / "annual_statistics" / f"{region.lower()}_annual_stats.csv")
...
@dataclass
class IndexRow:
    kind: str  # TABLE/FIGURE/DASHBOARD
    code: str
    title: str
    path_png: str
    path_pdf: str
...
def main():
    ...
    best_models = load_csv(BASE / "mhw_ml/outputs/metrics/best_models.csv")

    # ---------- Tables ----------
    # T01 Regional master summary
