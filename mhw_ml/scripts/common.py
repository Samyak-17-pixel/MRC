"""Shared utilities for MHW ML pipeline."""
...
def expand_climate_to_daily(daily_dates, climate_monthly, lag_months):
    ...
    # Monthly lags via shifting monthly table
...
def detect_event_starts(region):
    ...
    cat = pd.read_csv(
        cfg_path("results") / "mhw_catalogue" / f"{region}_mhw_catalogue.csv"
    )
...
def load_region_df(region):
    path = ML_ROOT / "data" / "processed" / f"{region}_daily_features.csv"
    df = pd.read_csv(path)
...
def ensure_dirs():
    for sub in [
        "data/processed", "data/splits", "models/baselines", "models/north",
        "models/central", "models/south", "outputs/metrics", "outputs/figures",
        "outputs/shap", "outputs/forecasts", "logs",
    ]:
