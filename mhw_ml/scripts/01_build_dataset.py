...
def main():
    ...
    for region in regions:
        ...
        out = ML_ROOT / "data" / "processed" / f"{region}_daily_features.csv"
    ...
    combined.to_csv(ML_ROOT / "data" / "processed" / "combined_daily_features.csv", index=False)
    ...
    for name, yrs in [("train", splits["train_years"]), ("val", splits["val_years"]), ("test", splits["test_years"])]:
        ...
        combined.loc[mask].to_csv(ML_ROOT / "data" / "splits" / f"{name}.csv", index=False)
        print(f"\n  Split {name}: {mask.sum()} rows ({yrs[0]}–{yrs[1]})")

    print("\n" + "=" * 72)
    print("DATASET BUILD COMPLETE")
    print(f"  Output: {ML_ROOT / 'data' / 'processed'}")
    print("=" * 72)
