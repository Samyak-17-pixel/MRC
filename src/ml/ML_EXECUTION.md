...
**Module location:** `src/ml/`  
**Last updated:** 2026-07-23 (paths updated for reorganization)

> Prefer [`README.md`](README.md) in this folder and the root [`README.md`](../../README.md) Part C.  
> This document describes **everything** the ML module does — data, labels, features, models, validation, outputs, and how to run it.
---
...
The ML module **does not duplicate** raw data. It reads from `outputs/`:

| Source | Path | Used for |
|--------|------|----------|
| Regional SST | `outputs/{region}_bob_sst.csv` | Core predictor |
| Hobday climatology | `outputs/mhw/climatology/{region}_hobday.csv` | Threshold, anomaly |
| Wind speed | `outputs/{region}_wind.csv` | Key driver (78–84% weak wind during MHWs) |
| Heat flux | `outputs/drivers/heat_flux/csv/{region}_slhf.csv`, `_sshf.csv` | Surface forcing |
| ONI | `outputs/climate_indices/enso/csv/oni_timeseries.csv` | ENSO |
| DMI | `outputs/climate_indices/iod/csv/dmi_timeseries.csv` | IOD (strongest driver) |
| MEI | `outputs/climate_indices/mei/csv/mei_timeseries.csv` | ENSO multivariate |
| MHW catalogue | `outputs/mhw/catalogue/{region}_mhw_catalogue.csv` | Event start dates (labels) |
...
```bash
cd /home/samyak/mrc_ws
.venv/bin/python src/ml/src/climate/scripts/run_pipeline.py
```

### Step by step

```bash
.venv/bin/python src/ml/src/climate/scripts/01_build_dataset.py
.venv/bin/python src/ml/src/climate/scripts/02_train_baselines.py
.venv/bin/python src/ml/src/climate/scripts/03_train_models.py
.venv/bin/python src/ml/src/climate/scripts/04_evaluate_models.py
.venv/bin/python src/ml/src/climate/scripts/05_explain_models.py
.venv/bin/python src/ml/src/climate/scripts/06_predict_current.py
```
