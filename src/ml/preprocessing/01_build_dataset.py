...
def build_region_dataset(region, config):
    ...
    sst = pd.read_csv(res / "timeseries" / f"{region}_bob_sst.csv")
    sst["Date"] = pd.to_datetime(sst["Date"])
    hob = pd.read_csv(res / "mhw" / "climatology" / f"{region}_hobday.csv")
    df = sst.copy()
    ...
    wind = pd.read_csv(res / "timeseries" / f"{region}_wind.csv")
    ...
    for flux, col in [("slhf", "SLHF"), ("sshf", "SSHF")]:
        fp = res / "drivers" / "heat_flux" / "csv" / f"{region}_{flux}.csv"
        if fp.exists():
    ...
    events = pd.read_csv(res / "mhw" / "catalogue" / f"{region}_mhw_catalogue.csv")
