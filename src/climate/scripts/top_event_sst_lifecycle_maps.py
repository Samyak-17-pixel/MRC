...
Output: outputs/top_event_sst_maps/
"""
...
def during_dates_strongest(start, end, region, regional_sst):
    ...
    hob = pd.read_csv(RESULTS / f"mhw/climatology/{region}_hobday.csv")
...
def process_event(ds, doy_clim, region, row, category, rank_metric_col, out_root):
    ...
    regional_sst = pd.read_csv(RESULTS / "timeseries" / f"{region}_bob_sst.csv")
